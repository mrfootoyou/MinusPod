...
NOT_AD_CLASSIFICATIONS = frozenset({
    'main_content', 'content', 'not_ad', 'editorial', 'organic',
...
<!--
Comments like this one are stripped before sending to the model.
-->
...
- Absorb musical cues, stingers, and short transition phrases into the following segment unless they serve as standalone transitions.
...
- `transition`: musical or narrative interludes, transition phrases ("this episode is brought to you by", "a word from our sponsors", "let's take a break")
- `main_content`: narrative, interview, Q&A
- `outro`: sign-off, credits, theme

**Category Confidence**

- `high`: very confident; ample evidence available
- `medium`: reasonably confident; some evidence available
- `low`: uncertain; little to no evidence.
...
- Guest plugs stay in `main_content`.
- Passing host mentions of URLs or social handles stay in `main_content` unless sustained and directed at the audience.
- When unsure if a segment qualifies as `main_content` or not, default to `main_content`.
...
{
 "segments": [
  {"start_id": 1, "end_id": 6, "category": "sponsor", "confidence": "high", "reason": "Ad for Whole Foods Market", "advertiser": "Whole Foods Market", "end_text": "Shop smarter, not harder"},
  {"start_id": 8, "end_id": 17, "category": "intro", "confidence": "high", "reason": "Cold open, welcome", "advertiser": null, "end_text": null},
  {"start_id": 18, "end_id": 130, "category": "main_content", "confidence": "high", "reason": null, "advertiser": null, "end_text": null},
  {"start_id": 130, "end_id": 134, "category": "outro", "confidence": "high", "reason": "Sign-off and credits", "advertiser": null, "end_text": null}
 ]
}
"""
