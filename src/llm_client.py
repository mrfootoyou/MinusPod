...
class OpenAICompatibleClient(LLMClient):
    ...
    def _get_json_schema_shared_with_model_supported(self, model: str) -> bool | None:
        """json_schema shared_with_model support for one model (#693)."""
    ...
    def _probe_format_support(self, kind: str, model: str | None) -> bool | None:
        ...
        systemPrompt = (
            "Answer the user when asked about this chat's response schema. "
            "If you don't have access to the response schema, then simply "
            "respond with the error: 'no response schema'."
            # Purposfully not mentioning JSON in order to test if the backend
            # forces a JSON response.
        )
        userPrompt = (
            "Regarding this chat's response schema: What is the description "
            "of property `foo`?"
        )
        responseSchema = """{
          "strict": true,
          "schema": {
            "description": "Respond with answer or err.",
            "type": "object",
            "properties": {
        ...
        probe_kwargs = {
            "model": model,
            token_param: 100, # should be plenty for the probe
            "messages": [
            ...
            "response_format": (
                {"type": "json_object"} if kind == 'json_object' else {
                    "type": "json_schema",
                    "json_schema": json.loads(responseSchema),
                }
            ),
        ...
        try:
            ...
            if choices and choices[0].message and choices[0].message.content:
                model_reply = choices[0].message.content
                try:
                    parsed = json.loads(model_reply)
                    # we got a parsable response
                    if kind == 'json_object':
                        # schema wasn't sent, so as long as the response is a
                        # dict, we're good
                        supported = isinstance(parsed, dict)
                    if kind == 'json_schema':
                        # expecting a JSON object with "answer" or "err"
                        # - Some providers enforce the schema AND automatically
                        #   share the schema with the model (OpenAI, Google).
                        #   In these cases there is no need to also include the
                        #   schema in our system prompt.
                        # - Other providers enforce the schema, but the model is
                        #   unaware of it (Ollama). In these cases we SHOULD
                        #   explicitly include the schema in our system prompt.
                        # - Other providers ignore the response format completely
                        #   (OpenRouter with Gemini 3.8).
                        answer = parsed.get("answer")
                        err = parsed.get("err")
                        if isinstance(answer, str) or isinstance(err, str):
                            supported = True
                            shared_with_model = isinstance(answer, str) and "pumpkin" in answer.lower()
        ...
        if supported:
            ...
            if shared_with_model is not None:
                logger.info(f"Schema is {'' if shared_with_model else 'NOT '}automatically shared with the model for {subject}")
        else:
            logger.info(
        ...
        if shared_with_model is not None:
            self._record_format_support(_JSON_SCHEMA_SHARED_WITH_MODEL_SETTING_KEY, model, shared_with_model)
        return supported
